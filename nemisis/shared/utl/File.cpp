...
namespace utl
{
	namespace
	{
		...
		public:
			...
			uint64_t Write(const void* buf, size_t size, size_t count) override {
				sizeTracker += size * count;
				return std::fwrite(buf, size, count, fptr);
			}
		...
		public:
			...
			//TODO
			uint64_t Write(const void*, size_t, size_t) override {
				return 0;
			}
