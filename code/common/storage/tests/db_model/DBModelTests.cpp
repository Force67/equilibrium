...
void CreateProject(database::SqliteDB& db,
                   const fs::path& path,
...
void CreateNodaDB(database::SqliteDB& db,
  ...
  {
    database::SqliteStatement s(
        db, "INSERT INTO file_root(prj_id, type, name, path) VALUES(?,?,?,?);");
    assert(s.Good());
  ...
  const protocol::Message* message =
      protocol::GetMessage(fbb.GetBufferPointer());
...
void DeleteProject(database::SqliteDB& hiveDb,
                   const fs::path& path,
                   const std::string& name) {
  assert(name.length() <= kMaxProjectNameSize);
...
void DumpAllUpdates(database::SqliteDB& db, const std::string& name) {
  auto statement = fmt::format("SELECT * FROM {}.updates", name);
  ...
  */
  ...
  while (dumpCommand.Step()) {
    auto string = fmt::format(
        "{}:{}:{}:{}", dumpCommand.ColumnInt(0), dumpCommand.ColumnStr(1),
        dumpCommand.ColumnBigInt(2), dumpCommand.ColumnBlob(3));
