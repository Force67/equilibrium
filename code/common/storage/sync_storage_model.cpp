...
bool RootDB::CreateProject(int32_t wksId,
...
bool NodaDB::AddUser(const std::string& name, const std::string& guid) {
  database::SqliteStatement command(
      _db, "insert into users (name, guid) VALUES(?,?)");
  if (!command.Good())
    return false;
...
bool NodaDB::AddUpdate(int32_t type,
                       int32_t user_id,
                       const void* data,
                       size_t dataLen) {
  database::SqliteStatement command(
      _db, "insert into updates (type, user_id, data) VALUES(?,?,?)");
  if (!command.Good())
    return false;
