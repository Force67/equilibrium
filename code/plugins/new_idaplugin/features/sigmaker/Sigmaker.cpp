...
namespace features {
  ...
  void GenerateSignature()
  {
	...
	for(int i = 0; i < kInstructionLimit; i++) {
	  ...
	  std::copy(instructionMask.begin(), instructionMask.end(), std::back_inserter(mask));

	  // Search from the min to the most address
	  if(bin_search2(inf.min_ea, address, bytes.data(), mask.data(), bytes.size(), BIN_SEARCH_FORWARD) == BADADDR) {
		// Search from the address we're making the pattern for + current pattern
		// size to the max-most address.
		if(bin_search2(address + bytes.size(), inf.max_ea, bytes.data(), mask.data(), bytes.size(), BIN_SEARCH_FORWARD) == BADADDR) {
		  break;
		}
	  }
