    ...
    _session(session)
{
  ...
  connect(&session, &SyncSession::TransportStateChange, this, [&](SyncSession::TransportState newState) {
...
void SyncService::SendSessionInfo()
{
  ...
  GNetClient()->SendPacket(protocol::MsgType_LocalProjectInfo, 
	  buffer, request.Union());
}
