...
namespace sync::make_code {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	return GNetClient()->SendPacket(protocol::MsgType_sync_MakeCode, buf, pack.Union());
  }
...
namespace sync::make_data {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	return GNetClient()->SendPacket(protocol::MsgType_sync_MakeData, buf, pack.Union());
  }
...
namespace sync::name_address {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	network::FbsBuffer buf;
	auto pack = CreateNameEaDirect(buf, addr, name,
	                               static_cast<bool>(local));

	return GNetClient()->
		SendPacket(protocol::MsgType_sync_NameEa, buf, pack.Union());
  }
