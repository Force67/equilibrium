...
namespace sync::change_item_comment {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	return GNetClient()->SendPacket(protocol::MsgType_sync_ChangeItemComment, buf, pack.Union());
  }
...
namespace sync::change_range_comment {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	network::FbsBuffer buf;
	auto pack = CreateChangeRangeCommentDirect(buf,
	            range->start_ea, kind, comment, rpt);

	return GNetClient()->SendPacket(protocol::MsgType_sync_ChangeRangeComment, buf, pack.Union());
  }
...
namespace sync::change_extra_comment {
  ...
  static bool React(SyncService &sc, va_list list)
  {
	...
	return GNetClient()->SendPacket(protocol::MsgType_sync_ChangeExtraComment, buf, pack.Union());
  }
