...
UiController::UiController(SyncController& s) : _sync(s) {
  ...
  _openFromServerAct =
      new QAction(QIcon(":/cloud_download"), "Open from Server", fileMenu);
  _saveToServerAct =
      new QAction(QIcon(":/cloud_upload"), "Save to server", fileMenu);
  connect(_openFromServerAct, &QAction::triggered, this,
          &UiController::OpenFromServer);
  connect(_saveToServerAct, &QAction::triggered, this,
          &UiController::SaveToServer);
  ...
  if (QMenu* nodaMenu = menuBar->addMenu("Noda")) {
    _connectAct =
        nodaMenu->addAction("Connect", this, &UiController::ToggleConnect);
    //_projectAct = nodaMenu->addAction(QIcon(":/sync"), "Projects", this,
    //&UiController::OpenSyncMenu);
    nodaMenu->addSeparator();
    nodaMenu->addAction(QIcon(":/cog"), "Settings", this,
                        &UiController::OpenSettings);
    nodaMenu->addSeparator();
    nodaMenu->addAction(QIcon(":/info"), "About NODA", this,
                        &UiController::OpenAboutDialog);
  }
  ...
  connect(&_sync, &SyncController::Connected, _netStatus.data(),
          &StatusWidget::OnConnected);
  connect(&_sync, &SyncController::Disconnected, _netStatus.data(),
          &StatusWidget::OnDisconnect);
  connect(&_sync, &SyncController::Announce, _netStatus.data(),
          &StatusWidget::OnAnnounce);
...
void UiController::DestroyUi() {
  // release the ownership of statusbar resources here early so ida cant release
  // them after our plugin is already unloaded
