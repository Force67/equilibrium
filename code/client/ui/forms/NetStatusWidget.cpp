...
namespace noda::ui
{
	...
	NetStatusWidget::NetStatusWidget(QWidget *parent) :
	    QWidget(parent)
	{
	...
	void NetStatusWidget::SetLabelIcon(QLabel &labelIcon, const QString &iconName)
	{
	...
	QSize NetStatusWidget::sizeHint() const
	{
		// connect
	...
	void NetStatusWidget::OnConnected()
	{
	...
	void NetStatusWidget::OnDisconnect(uint32_t r)
	{
	...
	void NetStatusWidget::OnStatsUpdate(const net::NetStats &stats)
	{
		_labelDownText->setText(QString::number(stats.bwDown));
		_labelUpText->setText(QString::number(stats.bwDown));
	}

	void NetStatusWidget::paintEvent(QPaintEvent *paintEvent)
	{
