...
template <typename CInsn, typename CInsnOp>
Capstone2LlvmIrTranslator_impl<CInsn,
                               CInsnOp>::~Capstone2LlvmIrTranslator_impl() {
  closeHandle();
}
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    setIgnoreUnexpectedOperands(bool f) {
  _ignoreUnexpectedOperands = f;
}

template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    setIgnoreUnhandledInstructions(bool f) {
  _ignoreUnhandledInstructions = f;
}

template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    setGeneratePseudoAsmFunctions(bool f) {
  _generatePseudoAsmFunctions = f;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn,
                                    CInsnOp>::isIgnoreUnexpectedOperands()
    const {
...
template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn,
                                    CInsnOp>::isIgnoreUnhandledInstructions()
    const {
...
template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn,
                                    CInsnOp>::isGeneratePseudoAsmFunctions()
    const {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::modifyBasicMode(
    cs_mode m) {
  if (!isAllowedBasicMode(m)) {
    throw ModeSettingError(_arch, m, ModeSettingError::eType::BASIC_MODE);
  }
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::modifyExtraMode(
    cs_mode m) {
  if (!isAllowedExtraMode(m)) {
    throw ModeSettingError(_arch, m, ModeSettingError::eType::EXTRA_MODE);
  }
...
template <typename CInsn, typename CInsnOp>
const csh& Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getCapstoneEngine()
    const {
  return _handle;
}

template <typename CInsn, typename CInsnOp>
cs_arch Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getArchitecture()
    const {
  return _arch;
}
...
template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::hasDelaySlot(
    uint32_t id) const {
  return false;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::hasDelaySlotTypical(
    uint32_t id) const {
  return false;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::hasDelaySlotLikely(
    uint32_t id) const {
  return false;
}

template <typename CInsn, typename CInsnOp>
std::size_t Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getDelaySlot(
    uint32_t id) const {
  return 0;
}
...
template <typename CInsn, typename CInsnOp>
llvm::GlobalVariable*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getRegister(uint32_t r) {
  auto fIt = _capstone2LlvmRegs.find(r);
  return fIt != _capstone2LlvmRegs.end() ? fIt->second : nullptr;
}

template <typename CInsn, typename CInsnOp>
std::string Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getRegisterName(
    uint32_t r) const {
  ...
  if (fIt == _reg2name.end()) {
    if (auto* n = cs_reg_name(_handle, r)) {
      ...
      throw GenericError("Missing name for register number: " +
                         std::to_string(r));
    }
  } else {
    return fIt->second;
...
template <typename CInsn, typename CInsnOp>
uint32_t Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getRegisterBitSize(
    uint32_t r) const {
  ...
  if (auto* it = llvm::dyn_cast<llvm::IntegerType>(rt)) {
    ...
    throw GenericError("Unhandled type of register number: " +
                       std::to_string(r));
  }
}
...
llvm::Type* Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getRegisterType(
  ...
  if (fIt == _reg2type.end()) {
    throw GenericError("Missing type for register number: " +
                       std::to_string(r));
  }
  return fIt->second;
}
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isControlFlowInstruction(
    cs_insn& i) const {
  return _controlFlowInsnIds.count(i.id) || isCallInstruction(i) ||
         isReturnInstruction(i) || isBranchInstruction(i) ||
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getCondBranchForInsnInIfThen(
  ...
  if (prevBb == nullptr || br == nullptr || !br->isConditional() ||
...
template <typename CInsn, typename CInsnOp>
llvm::Module* Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getModule()
    const {
  return _module;
}
...
template <typename CInsn, typename CInsnOp>
llvm::StoreInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isSpecialAsm2LlvmInstr(
    llvm::Value* v) const {
...
template <typename CInsn, typename CInsnOp>
llvm::GlobalVariable*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getAsm2LlvmMapGlobalVariable()
    const {
  return _asm2llvmGv;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isCallFunction(
    llvm::Function* f) const {
  return f == _callFunction;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isCallFunctionCall(
...
template <typename CInsn, typename CInsnOp>
llvm::BranchInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isInConditionCallFunctionCall(
    llvm::CallInst* c) const {
...
template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getCallFunction() const {
  return _callFunction;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isReturnFunction(
    llvm::Function* f) const {
...
template <typename CInsn, typename CInsnOp>
llvm::BranchInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isInConditionReturnFunctionCall(
    llvm::CallInst* c) const {
...
template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getReturnFunction() const {
  return _returnFunction;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isBranchFunction(
    llvm::Function* f) const {
...
template <typename CInsn, typename CInsnOp>
llvm::BranchInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isInConditionBranchFunctionCall(
    llvm::CallInst* c) const {
...
template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getBranchFunction() const {
...
template <typename CInsn, typename CInsnOp>
llvm::BranchInst* Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    isInConditionCondBranchFunctionCall(llvm::CallInst* c) const {
  return isCondBranchFunctionCall(c) ? getCondBranchForInsnInIfThen(c)
                                     : nullptr;
}

template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getCondBranchFunction() const {
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isAnyPseudoFunction(
    llvm::Function* f) const {
  return isCallFunction(f) || isReturnFunction(f) || isBranchFunction(f) ||
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isAnyPseudoFunctionCall(
    llvm::CallInst* c) const {
  return isCallFunctionCall(c) || isReturnFunctionCall(c) ||
         isBranchFunctionCall(c) || isCondBranchFunctionCall(c);
}

template <typename CInsn, typename CInsnOp>
llvm::GlobalVariable*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isRegister(
    llvm::Value* v) const {
  auto it =
      _llvm2CapstoneRegs.find(llvm::dyn_cast_or_null<llvm::GlobalVariable>(v));
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunctions() const {
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::initialize() {
  if (!isAllowedBasicMode(_basicMode)) {
    throw ModeSettingError(_arch, _basicMode,
                           ModeSettingError::eType::BASIC_MODE);
  }
  if (!isAllowedExtraMode(_extraMode)) {
    throw ModeSettingError(_arch, _extraMode,
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
  ...
  _asm2llvmGv = new llvm::GlobalVariable(*_module, t,
                                         false,  // isConstant
                                         lt, initializer);
}
...
template <typename CInsn, typename CInsnOp>
llvm::StoreInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateSpecialAsm2LlvmInstr(
    llvm::IRBuilder<>& irb,
    cs_insn* i) {
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCallFunction() {
  auto* ft = llvm::FunctionType::get(
      ...
      {llvm::Type::getIntNTy(_module->getContext(), getArchBitSize())}, false);
  _callFunction = llvm::Function::Create(
      ft, llvm::GlobalValue::LinkageTypes::ExternalLinkage, "", _module);
}

template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCallFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* t) {
...
template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCondCallFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* cond,
    llvm::Value* t) {
...
template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateReturnFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* t) {
...
template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCondReturnFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* cond,
    llvm::Value* t) {
  auto bodyIrb = generateIfThen(cond, irb);
...
template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateBranchFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* t) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn,
                                    CInsnOp>::generateCondBranchFunction() {
  std::vector<llvm::Type*> params = {
      llvm::Type::getInt1Ty(_module->getContext()),
      llvm::Type::getIntNTy(_module->getContext(), getArchBitSize())};
  auto* ft = llvm::FunctionType::get(
      llvm::Type::getVoidTy(_module->getContext()), params, false);
...
template <typename CInsn, typename CInsnOp>
llvm::CallInst*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCondBranchFunctionCall(
    llvm::IRBuilder<>& irb,
    llvm::Value* cond,
    llvm::Value* t) {
...
template <typename CInsn, typename CInsnOp>
llvm::GlobalVariable*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::createRegister(
    uint32_t r,
  ...
  auto* gv = new llvm::GlobalVariable(*_module, rt,
                                      false,  // isConstant
                                      lt, initializer, getRegisterName(r));
...
llvm::Value* Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOp(
    ...
    eOpConv ct) {
  if (ci->op_count <= idx) {
    throw GenericError("Idx out of bounds: " + std::to_string(idx) + "/" +
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::_loadOps(CInsn* ci,
                                                         llvm::IRBuilder<>& irb,
                                                         std::size_t opCnt,
                                                         bool strict,
                                                         llvm::Type* loadType,
  ...
  if ((strict && (ci->op_count != opCnt)) || (ci->op_count < opCnt)) {
    throw GenericError("Trying to load " + std::to_string(opCnt) +
                       " operands from instruction with" +
                       std::to_string(ci->op_count) + " opernads.");
  }
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::_loadOpsUniversal(
  ...
  if (op0->getType()->isIntegerTy()) {
    auto operands =
        _loadOps(ci, irb, opCnt - 1, false, nullptr, op0->getType(), ict);
    operands.insert(operands.begin(), op0);
    return operands;
  }

  auto operands =
      _loadOps(ci, irb, opCnt - 1, false, nullptr, op0->getType(), fct);
  operands.insert(operands.begin(), op0);
  return operands;
}
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCarryAddInt4(
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb) {
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCarryAddCInt4(
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb,
    llvm::Value* cf) {
  ...
  if (cf == nullptr) {
    cf = loadRegister(getCarryRegister(), irb, a->getType(),
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowAdd(
    llvm::Value* add,
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb) {
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowAddC(
    llvm::Value* add,
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb,
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowSub(
    llvm::Value* sub,
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb) {
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowSubC(
    llvm::Value* sub,
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb,
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateBorrowSubInt4(
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb) {
  auto* ci15 = llvm::ConstantInt::get(op0->getType(), 15);
...
template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateBorrowSubCInt4(
    llvm::Value* op0,
    llvm::Value* op1,
    llvm::IRBuilder<>& irb,
    llvm::Value* cf) {
...
template <typename CInsn, typename CInsnOp>
llvm::IntegerType*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getDefaultType() {
...
template <typename CInsn, typename CInsnOp>
std::string
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunctionName(
    cs_insn* insn) {
...
template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunction(
    cs_insn* insn,
  ...
  if (fIt == _insn2asmFunctions.end()) {
    auto* fnc = llvm::Function::Create(
        type, llvm::GlobalValue::LinkageTypes::ExternalLinkage, n, _module);
...
/**
 * The same as @c getPseudoAsmFunction(std::size_t,std::string&,
 * llvm::FunctionType*), but function type is created by this variant.
 */
template <typename CInsn, typename CInsnOp>
llvm::Function*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunction(
  ...
  return getPseudoAsmFunction(
      insn, llvm::FunctionType::get(retType, params, false), name);
}

template <typename CInsn, typename CInsnOp>
llvm::Value*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateTypeConversion(
    llvm::IRBuilder<>& irb,
    llvm::Value* from,
    llvm::Type* to,
    eOpConv ct) {
  ...
  switch (ct) {
    case eOpConv::SEXT_TRUNC_OR_BITCAST: {
      if (!to->isIntegerTy()) {
        throw GenericError(
            "Invalid combination of conversion method and destination type");
      }
      ...
      if (from->getType()->isIntegerTy()) {
        ...
        auto size =
            _module->getDataLayout().getTypeStoreSizeInBits(from->getType());
    ...
    case eOpConv::FPCAST_OR_BITCAST: {
      ...
      if (from->getType()->isFloatingPointTy()) {
        ...
        auto isize =
            _module->getDataLayout().getTypeStoreSizeInBits(from->getType());
        auto dsize =
            _module->getDataLayout().getTypeStoreSizeInBits(irb.getDoubleTy());
        auto fsize =
            _module->getDataLayout().getTypeStoreSizeInBits(irb.getFloatTy());
        auto lsize = _module->getDataLayout().getTypeStoreSizeInBits(
            llvm::Type::getFP128Ty(_module->getContext()));
        ...
        if (isize == fsize) {
          ...
          from = irb.CreateBitCast(
              from, llvm::Type::getFP128Ty(_module->getContext()));
        } else {
          throw GenericError(
              "Unable to create bitcast to floating point type.");
        }
...
template <typename CInsn, typename CInsnOp>
llvm::Type*
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::_checkTypeConversion(
    llvm::IRBuilder<>& irb,
    llvm::Type* to,
    eOpConv ct) {
  switch (ct) {
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::translatePseudoAsmOp0Fnc(
  ...
  llvm::Function* fnc =
      getPseudoAsmFunction(i, getDefaultType(), llvm::ArrayRef<llvm::Type*>{});
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::translatePseudoAsmFncOp0(
  ...
  llvm::Function* fnc = getPseudoAsmFunction(
      i, irb.getVoidTy(), llvm::ArrayRef<llvm::Type*>{op0->getType()});
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp0(cs_insn* i, CInsn* ci, llvm::IRBuilder<>& irb) {
  EXPECT_IS_UNARY(i, ci, irb);

  op0 = loadOp(ci->operands[0], irb);

  llvm::Function* fnc = getPseudoAsmFunction(
      i, op0->getType(), llvm::ArrayRef<llvm::Type*>{op0->getType()});
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmFncOp0Op1(cs_insn* i, CInsn* ci, llvm::IRBuilder<>& irb) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp1(cs_insn* i, CInsn* ci, llvm::IRBuilder<>& irb) {
  EXPECT_IS_BINARY(i, ci, irb);

  op1 = loadOp(ci->operands[1], irb);

  llvm::Function* fnc = getPseudoAsmFunction(
      i, op1->getType(), llvm::ArrayRef<llvm::Type*>{op1->getType()});
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp0Op1(cs_insn* i,
                                   CInsn* ci,
                                   llvm::IRBuilder<>& irb) {
  EXPECT_IS_BINARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmFncOp0Op1Op2(cs_insn* i,
                                   CInsn* ci,
                                   llvm::IRBuilder<>& irb) {
  ...
  llvm::Function* fnc =
      getPseudoAsmFunction(i, irb.getVoidTy(),
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp1Op2(cs_insn* i,
                                   CInsn* ci,
                                   llvm::IRBuilder<>& irb) {
  EXPECT_IS_TERNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp0Op1Op2(cs_insn* i,
                                      CInsn* ci,
                                      llvm::IRBuilder<>& irb) {
  EXPECT_IS_TERNARY(i, ci, irb);
  ...
  llvm::Function* fnc =
      getPseudoAsmFunction(i, op0->getType(),
                           llvm::ArrayRef<llvm::Type*>{
                               op0->getType(), op1->getType(), op2->getType()});
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmFncOp0Op1Op2Op3(cs_insn* i,
                                      CInsn* ci,
                                      llvm::IRBuilder<>& irb) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp1Op2Op3(cs_insn* i,
                                      CInsn* ci,
                                      llvm::IRBuilder<>& irb) {
  EXPECT_IS_QUATERNARY(i, ci, irb);
  ...
  llvm::Function* fnc =
      getPseudoAsmFunction(i, getDefaultType(),
                           llvm::ArrayRef<llvm::Type*>{
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp0Op1Op2Op3(cs_insn* i,
                                         CInsn* ci,
                                         llvm::IRBuilder<>& irb) {
  EXPECT_IS_QUATERNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0Op1FncOp0Op1Op2Op3(cs_insn* i,
                                            CInsn* ci,
                                            llvm::IRBuilder<>& irb) {
  EXPECT_IS_QUATERNARY(i, ci, irb);
 ...
 */
template <typename CInsn, typename CInsnOp>
uint8_t Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getOperandAccess(
    CInsnOp&) {
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::translatePseudoAsmGeneric(
  ...
  llvm::Function* fnc = getPseudoAsmFunction(i, retType, types);
  ...
  for (auto r : writeRegs) {
    llvm::Value* val =
        retType->isVoidTy()
            ? llvm::cast<llvm::Value>(llvm::UndefValue::get(getRegisterType(r)))
