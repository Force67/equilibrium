...
TEST(GridLayout, LayoutWithChildren) {
  ...
  // first row bounds
  EXPECT_EQ(g.children_[0]->bounds, ui::FPoint::Make(10.f, 60.f));
  EXPECT_EQ(g.children_[1]->bounds, ui::FPoint::Make(20.f, 60.f));
  EXPECT_EQ(g.children_[2]->bounds, ui::FPoint::Make(30.f, 60.f));

  // second row bounds
  EXPECT_EQ(g.children_[3]->bounds, ui::FPoint::Make(10.f, 70.f));
  EXPECT_EQ(g.children_[4]->bounds, ui::FPoint::Make(20.f, 70.f));
  EXPECT_EQ(g.children_[5]->bounds, ui::FPoint::Make(30.f, 70.f));

  // 3 colums x 3 rows would give 9; leave out 3rd row deliberatly

  // remember; gap = row_gap; column_gap
  EXPECT_EQ(g.children_[0]->screen_pos, ui::FPoint::Make(0.f, 0.f));
  ...
  EXPECT_EQ(g.children_[5]->screen_pos,
