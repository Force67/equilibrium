...
TEST(CRC32CTest, HardwareCRC32) {
  ...
  uint32_t crc1 = CRC32C(reinterpret_cast<const uint8_t*>(data1),
                         sizeof(data1) - 1);  // Subtract 1 to avoid null terminator
  EXPECT_EQ(crc1, 0xa25caaff);

  const char data2[] = "45";
  uint32_t crc2 =
      CRC32C(reinterpret_cast<const uint8_t*>(data2), sizeof(data2) - 1, ~crc1);
  EXPECT_EQ(crc2, 0x9e63ca4c);
}

TEST(CRC32CTest, SoftwareCRC32) {
  const char data1[] = "123";
  uint32_t crc1 = CRC32C_SW(reinterpret_cast<const uint8_t*>(data1), sizeof(data1) - 1);
...
TEST(CRC32CTest, HardwareAndSoftwareConsistency) {
  ...
  uint32_t crc_sw = CRC32C_SW(reinterpret_cast<const uint8_t*>(data), sizeof(data) - 1);
...
TEST(CRC32Test, SoftwareCRC32) {
  ...
  uint32_t crc1 = CRC32_SW(reinterpret_cast<const uint8_t*>(data1), sizeof(data1) - 1);
