...
//
// It is similar to std::optional, but std::optional can only indicate a normal value
// or std::nullopt, i.e. a null value. In contrast, std::expected can indicate an
// expected value and an error value, which is equivalent to the two-member
// std::variant, but is more convenient to use on the interface.
#pragma once
...
class Expected {
 ...
 public:
  ...
  inline Expected(U value) requires(!base::ISSame<U, TError>)
      : has_error_(false), expected_value_(base::move(value)) {}

  template <typename U = TError>
  inline Expected(U error) requires(!base::ISSame<U, TExpected>)
