...
#pragma once
// Port for fbstuff by force
#include <base/compiler.h>
#include <base/check.h>

#define FOLLY_NOINLINE NOINLINE
#define FOLLY_ALWAYS_INLINE STRONG_INLINE
#define FOLLY_UNLIKELY UNLIKELY
#define FOLLY_LIKELY LIKELY
...
FOLLY_NOINLINE inline void* smartRealloc(void* p,
...
class fbstring_core {
 ...
 private:
  ...
  struct RefCounted {
    ...
    static RefCounted* reallocate(Char* const data,
      ...
      auto result = static_cast<RefCounted*>(smartRealloc(
          dis, getDataOffset() + (currentSize + 1) * sizeof(Char),
  ...
  enum class Category : category_type {
    isSmall = 0,
    isMedium = base::kIsLittleEndian ? 0x80 : 0x2,
    isLarge = base::kIsLittleEndian ? 0x40 : 0x1,
  };
  ...
  constexpr static uint8_t categoryExtractMask = base::kIsLittleEndian ? 0xC0 : 0x3;
...
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::initMedium(const Char* const data,
                                                    const size_t size) {
...
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::initLarge(const Char* const data,
                                                   const size_t size) {
...
FOLLY_NOINLINE void fbstring_core<Char>::reserveMedium(const size_t minCapacity) {
  ...
  if (minCapacity <= maxMediumSize) {
    ...
    // Also copies terminator.
    ml_.data_ = static_cast<Char*>(smartRealloc(ml_.data_, (ml_.size_ + 1) * sizeof(Char),
...
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::reserveSmall(size_t minCapacity,
...
basic_fbstring<E, T, A, S>::traitsLength(const value_type* s) {
  return s ? traits_type::length(s) : -1;
}
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A, S>::append(size_type n,
                                                                      value_type c) {
  Invariant checker(*this);
  auto pData = store_.expandNoinit(n, /* expGrowth = */ true);
  fbstring_detail::podFill(pData, pData + n, c);
...
constexpr
    typename basic_fbstring<E1, T, A, S>::size_type basic_fbstring<E1, T, A, S>::npos;
