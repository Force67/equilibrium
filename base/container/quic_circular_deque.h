...
class QuicCircularDeque {
 ...
 public:
  ...
  reference front() { return *index_to_address(begin_); }
 ...
 private:
  ...
  // with the fields we had to store anyway, via inheriting from the
  // allocator, so this allocator instance doesn't consume any storage when
  // its type has no data members.
