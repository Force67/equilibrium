...
//
// BasicSmallString<TChar, N>: like BasicBaseString but with a templated
// inline buffer of N characters. Spills onto the heap via TAllocator when it
// outgrows N. Use when values are usually larger than BasicBaseString's
// 22-byte SSO buffer but heap traffic is still unwanted.
//
// Layout:
...
//
// Deliberately no bit-stuffing: for large N the bookkeeping buys nothing and
// the explicit bool is unambiguous.
#pragma once
...
template <typename TChar, mem_size N, class TAllocator = base::DefaultAllocator>
class BasicSmallString {
 ...
 private:
  ...
  size_type get_size() const noexcept { return is_large_ ? large_.size_ : small_.size_; }
  ...
  void grow_to(size_type required) {
    ...
    if (cur_size > 0) {
      base::MemCopy(new_data, const_data(), cur_size * sizeof(character_type));
    }
 ...
 public:
  ...
  BasicSmallString(const character_type (&arr)[ArraySize]) {
    init_empty();
    assign(arr, static_cast<size_type>(
  ...
  BasicSmallString(BasicSmallString&& other) noexcept {
    if (other.is_large_) {
      ...
      base::MemCopy(small_.data_, other.small_.data_,
  ...
  void assign(const character_type* str, size_type len) {
    ...
    if (len > get_capacity())
      grow_to(len);
    base::MemCopy(mutable_data(), str, len * sizeof(character_type));
    set_size(len);
    terminate();
  }
  ...
  const character_type* end() const noexcept { return const_data() + get_size(); }
  ...
  void resize(size_type new_size) {
    ...
    if (new_size > old_size) {
      reserve(new_size);
      base::MemSet(mutable_data() + old_size, 0,
  ...
  void shrink_to_fit() {
    ...
    if (cur <= kInlineCapacity) {
      ...
      is_large_ = false;
      small_.size_ = cur;
      base::MemCopy(small_.data_, old_data, cur * sizeof(character_type));
      ...
      base::MemCopy(new_data, large_.data_, cur * sizeof(character_type));
  ...
  void append(const character_type* str, size_type count) {
    ...
    if (new_size > get_capacity())
      grow_to(new_size);
    base::MemCopy(mutable_data() + old_size, str, count * sizeof(character_type));
    set_size(new_size);
    terminate();
  }
  ...
  void erase(size_type pos = 0, size_type count = npos) {
    ...
    if (count == 0)
      return;
    character_type* d = mutable_data();
    base::MemMove(d + pos, d + pos + count, (cur - pos - count) * sizeof(character_type));
    set_size(cur - count);
    terminate();
  }
  ...
  int compare(const character_type* str, size_type count) const noexcept {
    ...
    int result = base::MemCompare(const_data(), str, min_size * sizeof(character_type));
    if (result != 0)
      return result;
    if (left_size < count)
      return -1;
...
template <typename TChar, mem_size N, class TAllocator>
bool operator==(const BasicSmallString<TChar, N, TAllocator>& lhs, const TChar* rhs) {
  if (rhs == nullptr)
    return lhs.empty();
